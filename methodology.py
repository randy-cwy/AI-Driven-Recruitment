...
def methodology_page():
    ...
    st.markdown("""
            <p class="methodology-paragraph">Below, we outline the data flow, implementation steps, and key design choices that guided the development of this solution.</p>
    """, unsafe_allow_html=True)
    ...
    # Tab 1: Data Flow & Implementation
    with tab1:

        st.markdown("""
        <p class="methodology-paragraph">The project's data flow involves structured steps, beginning with parsing the job description and proceeding through candidate evaluation, scoring, and assessment generation. Implementation details are also covered in this section. Click on each tab below to view the different stages of the program.</p>
        """, unsafe_allow_html=True)

        tab6, tab7, tab8, tab9, tab10, tab11 = st.tabs(
            [
                "1️⃣ Step 1",
                "2️⃣ Step 2",
                "3️⃣ Step 3",
        ...
        with tab6:
            st.markdown("""
                <div class="methodology-subheader">Step 1: Job Description Parsing (<code>helper/file_handler.py</code>)</div>
        ...
        with tab7:
            st.markdown("""
                <div class="methodology-subheader">Step 2: SkillsFuture Framework Matching (<code>helper/skills_mapping.py</code>)</div>
                <ul class="methodology-list">
        ...
        with tab8:
            st.markdown("""
                <div class="methodology-subheader">Step 3: Candidate Resume Processing (<code>helper/bulk_resume_processor.py</code>)</div>
                <ul class="methodology-list">
                    ...
                    <li><b>Text Extraction:</b> For each resume, text extraction is tailored based on file type. <code>.docx</code> files are read directly, while <code>.pdf</code> files are processed using <code>PyPDF2</code> to accurately extract the text for analysis.</li>
        ...
        with tab9:
            st.markdown("""
                <div class="methodology-subheader">Step 4: Candidate Scoring and Ranking (<code>helper/scoring.py</code>)</div>
                <ul class="methodology-list">
        ...
        with tab10:
            st.markdown("""
                <div class="methodology-subheader">Step 5: Candidate Filtering (<code>streamlit_app.py</code>)</div>
                <ul class="methodology-list">
                    <li>After scoring, candidates are ranked based on their total scores, allowing the user to specify the number of top candidates to display.
        ...
        with tab11:
            st.markdown("""
                <div class="methodology-subheader">Step 6: Assessment Generation and Answer Key (<code>helper/assessment_generator.py</code>)</div>
                <ul class="methodology-list">
                    <li>The system generates tailored assessments for each of the top X candidates based on the selected number in the previous filtering step.</li>
    ...
    # Tab 2: Design Considerations
    with tab2:
        st.markdown("""
        <ul class="methodology-list">
            <div class="methodology-subheader">Avoidance of Prompt Chaining</div>
            <ul class="methodology-list">
                <li><b>Increased Computation Time:</b> Manual prompt chaining noticeably extended computation time, as each additional prompt cycle accumulated processing delays. To keep the user experience responsive, I prioritized reducing these delays, especially since the difference in results (with/without prompt chaining) was not significant.</li>
                <li><b>Token Limits and Truncated Responses:</b> With LangChain, I frequently encountered token limits, resulting in responses that were often cut off. This made LangChain unreliable for handling longer job descriptions and multiple resumes.</li>
            </ul>
            <div class="methodology-subheader"; style="margin-top: 10px;">Exclusion of Agents</div>
            <ul class="methodology-list">
                <li><b>Linear Workflow:</b> This project follows a straightforward sequence—parsing job descriptions, matching skills, scoring candidates, generating assessments. Each step is handled once with static inputs rather than continuously updated data, so an iterative agent wasn't necessary.</li>
                <li><b>No Dynamic Adaptation Needed:</b> The workflow doesn't require adjustments based on prior actions or feedback, as each step is independent. Since there are no branching paths or real-time decision-making, agents would add unnecessary complexity.</li>
                <li><b>Clear, Predefined Steps:</b> With a clear pathway and no large state space of actions to explore, the project didn't need agents to handle alternative strategies or complex decision flows.</li>
            </ul>
            <div class="methodology-subheader"; style="margin-top: 10px;">Prompt Injection Protections Not Required</div>
            <ul class="methodology-list">
            ...
            <div class="methodology-subheader"; style="margin-top: 10px;">User Experience (UX) Optimization</div>
            <ul class="methodology-list">
            ...
            <div class="methodology-subheader"; style="margin-top: 10px;">Performance Efficiency</div>
            <ul class="methodology-list">
